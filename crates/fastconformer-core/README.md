...
## Encoder forward — validated against NeMo

The encoder (subsample → xscale → N × ConformerBlock) is verified
end-to-end against NeMo's `ConformerEncoder` on
`samples/japanese_test.wav`: the Rust pipeline emits the same 27
token IDs as NeMo's RNN-T greedy/beam decode, producing
`吾輩は猫である名前はまだないどこで生まれたかとんと見当がつかぬ。`.

Reference dumps from a Python 3.11 venv with `nemo-toolkit[asr]` are
generated by `scripts/nemo-truth-dump.py` (full per-block outputs)
and `scripts/nemo-block-internals-dump.py` (per-stage internals
inside block 0). Comparison harness lives in
`crates/reazonspeech-backend/tests/encoder_ref_decode.rs`.

Key fixes that landed during validation:
- `xscale = sqrt(d_model)` applied to the subsample output (`PositionalEncoding.forward`)
- No final encoder-level `ln_post` (it doesn't exist in NeMo; each block has its own `norm_out`)
- Longformer global queries score with AC only (no rel-pos BD)
- Depthwise-conv `pad_mask` zeros the trailing subsample frame
- Mel `log_zero_guard = 2⁻²⁴` (not 1e-5) and Bessel-corrected std
- RNN-T joint activation is **relu** (not tanh)
- RNN-T initial `pred_h` runs the LSTM once with zero embedding so
  the gate biases shape the start state
